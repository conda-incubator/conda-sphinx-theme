...
@pytest.fixture
def version_pattern():
    """Fixture providing the compiled version regex pattern."""
    return re.compile(r"^(\d+\.\d+(?:\.\d+)?)\s*\(.*?\)$")
...
def mock_config(mocker):
    ...
    config.version_anchor_changelog_files = ["changelog", "release", "history", "news"]
    return config


# Pattern matching tests
@pytest.mark.parametrize("heading_text,expected_version", [
...
def test_version_pattern_matches(version_pattern, heading_text, expected_version):
    """Test that version patterns match expected formats."""
    match = version_pattern.match(heading_text)
...
def test_version_pattern_non_matches(version_pattern, heading_text):
    """Test that version patterns reject invalid formats."""
    match = version_pattern.match(heading_text)
    assert match is None, f"Pattern should NOT match '{heading_text}'"


# Anchor format generation tests
@pytest.mark.parametrize("version,format_template,expected_anchor", [
...
def test_changelog_file_detection(mocker):
    ...
    document.settings.env.app.config.version_anchor_changelog_files = ["changelog", "release", "history", "news"]
...
@pytest.mark.parametrize("pattern,format_str,heading,expected_id", [
    (r"^Version\s+(\d+\.\d+(?:\.\d+)?).*$", "v{version}", "Version 1.2.3", "v1.2.3"),
    (r"^Release\s+(\d+\.\d+(?:\.\d+)?)", "release-{version}", "Release 2.0", "release-2.0"), 
    (r"^v?(\d+\.\d+(?:\.\d+)?)", "{version}", "v3.1.4", "3.1.4"),
    (r"^(\d+\.\d+(?:\.\d+)?)\s*\(.*?\)$", "version-{version}", "1.0.0 (Final)", "version-1.0.0"),
])
